...
import { Scissors, Plus, Trash2, CheckCircle, Package } from "lucide-react";
...
export default function ServiceConsumptionRules({ products = [], onRefresh }) {
  ...
  const fetchData = async () => {
    try {
      ...
      const [ruleRes, serviceRes] = await Promise.all([
  ...
  return (
    <Row className="g-4 animate-fade-in">
                  ...
                  <tbody style={{ fontSize: "13px" }}>
                    {rules.map(r => (
                      <tr key={r.id} className="transition-all hover-row-focus">
                        <td className="ps-3 py-2.5 fw-bold text-gray-900">
                        ...
                        <td className="py-2.5 text-center fw-bold text-gray-800">
      ...
      {/* Rules Creation Form (Right column) */}
      <Col lg={4}>
