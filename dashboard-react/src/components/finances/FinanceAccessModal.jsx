...
export default function FinanceAccessModal({ show, onHide, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await api.post("/auth/finance-access", {
        username: username.trim(),
        password,
        businessId: business?.id || localStorage.getItem("active_business_id") || "default"
      });

      if (res.data?.success && res.data?.token) {
        unlockFinance(res.data.token);
        onSuccess?.();
        setUsername("");
        setPassword("");
      } else {
      ...
      console.error("Finance access verification error:", err);
      setError(err.response?.data?.error || "Error al verificar las credenciales del supervisor.");
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      ...
      >
        <Modal.Header className="border-0 bg-transparent py-4 text-center d-flex flex-column align-items-center">
          ...
          <Modal.Title className="h4 fw-black text-dark block-text">Acceso Restringido</Modal.Title>
          <p className="text-secondary small px-3 mt-2 mb-0" style={{ lineHeight: "1.5" }}>
            Esta sección contiene información financiera sensible del negocio. Ingresa credenciales de supervisor autorizadas para continuar.
          </p>
        </Modal.Header>
        ...
        <Form onSubmit={handleSubmit}>
          <Modal.Body className="bg-transparent px-4 py-2">
            ...
            <Form.Group className="mb-3">
              <Form.Label htmlFor="supervisor-username" className="fw-bold text-dark small mb-1 d-flex align-items-center gap-1.5">
            ...
            <Form.Group className="mb-3">
              <Form.Label htmlFor="supervisor-password" className="fw-bold text-dark small mb-1 d-flex align-items-center gap-1.5">
